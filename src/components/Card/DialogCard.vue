<template>
  ...
  >
    <component :is="item.component" v-for="item in elem.items"
      ...
      @change="handleChanged(item)"
      @keydown.enter="handleEnter({'event':$event.target.value,'item':item})"
      v-model="item.value"
...
// import CardCarousel from "@/components/Carousel/CardCarousel.vue";   //conflict with Section for unknown reason
