<template>
  <div
    :style="elem.style"
  >
    <v-main :style="elem.value?'height:100vh;width:100vw;':'height:min-content;width:min-content;'">
      <v-app-bar-nav-icon @click.stop="elem.value = !elem.value"/>
    </v-main>
    <v-navigation-drawer 
...
// import grapozaTreeView from "@/components/Tree/grapozaTreeView.vue";
import n00tsTreeView from "@/components/Tree/n00tsTreeView.vue";
  ...
  mounted() {
    if (this.$root.methodList !== null && this.$root.methodList.length > 0) {
      this.$root.methodList.forEach(function (item) {
        if (item.component === "NavigationDrawer") {
